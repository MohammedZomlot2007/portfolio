...
/* ---------- FX intensity (Full/Lite — controls 3D & motion weight) ---------- */
function currentFx() {
  var saved = null;
  try { saved = localStorage.getItem('mz-fx'); } catch (e) {}
  if (saved) return saved;
  var cfg = (window.MZ_CONFIG && window.MZ_CONFIG.fxDefault) || 'full';
  // auto-default to lite on low-memory / reduced-motion devices, unless user already chose
  if (prefersReduced) return 'lite';
  if (navigator.deviceMemory && navigator.deviceMemory <= 4) return 'lite';
  return cfg;
}
function applyFx(v) {
  root.setAttribute('data-fx', v);
  try { localStorage.setItem('mz-fx', v); } catch (e) {}
}
applyFx(currentFx());
var fxBtn = $('#fx-toggle');
if (fxBtn) fxBtn.addEventListener('click', function () {
  var next = root.getAttribute('data-fx') === 'lite' ? 'full' : 'lite';
  applyFx(next);
  toast(next === 'lite' ? 'Lite mode · مؤثرات أخف' : 'Full effects · كل المؤثرات');
  // hero/stack 3D scenes read data-fx only at their own init time, so a live
  // toggle takes full effect after the next page load — never breaks anything now.
});
...
var __ps = $('#project-search');
if (__ps) __ps.addEventListener('input', function (e) { searchTerm = e.target.value; renderProjects(); });
...
function addDepthMotion(cards){
  cards.forEach(function(el, index){
    if (el.classList.contains('mz-depth')) return;
    el.classList.add('mz-depth');
    el.style.setProperty('--mz-float-duration', (5.8 + (index % 5) * .72).toFixed(2) + 's');
    el.style.setProperty('--mz-float-delay', (-index * .37).toFixed(2) + 's');
  });
}
window.__mzEnhance3D=initSite3D;

/* ---------- Add-on i18n keys (FAQ + CV) ---------- */
I18N.en.navLab = '3D Lab';
I18N.en.achBtn = 'Open the interactive timeline';
I18N.en.wkTease = 'All ten works now live on their own interactive timeline page — horizontal scroll, one case study at a time.';
I18N.en.wkBtn = 'Open the Works timeline';
I18N.en.navFaq = 'FAQ';
...
I18N.ar.navLab = 'مختبر 3D';
I18N.ar.achBtn = 'افتح الخط الزمني التفاعلي';
I18N.ar.wkTease = 'الأعمال العشرة تعيش الآن في صفحة خط زمني تفاعلية خاصة — سكرول أفقي، دراسة حالة واحدة كل مرة.';
I18N.ar.wkBtn = 'افتح خط الأعمال الزمني';
...
/* ---------- Split-text hero title (letters cascade in via the existing
   .reveal / IntersectionObserver system — no extra observer needed) ---------- */
function splitHeroTitle() {
  var cfg = (window.MZ_CONFIG && window.MZ_CONFIG.splitText) || { enabled: true, staggerMs: 22 };
  if (!cfg.enabled || prefersReduced) return;
  $$('.hero-title .line.reveal').forEach(function (line) {
    if (line.dataset.split) return; // never re-split, keeps this idempotent/safe
    ...
    text.split('').forEach(function (ch) {
      ...
      span.style.transitionDelay = (i * cfg.staggerMs) + 'ms';
